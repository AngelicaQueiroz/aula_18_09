public class Exemplo6Estados {
    public static void main(String[] args) {
        ...
        //Iniciando a thread
        thread.start();
        //Estado: RUNNABLE (Executável) - A thread está em execução ou pronta para ser executada
        System.out.println("Estado da thread:"+ thread.getState());
        try {
...
class MinhaThread implements Runnable {
    ...
    public void run() {
        ...
        try {
            //Estado: TIMED_WAITING (Aguardando tempo determinado) - A thread está esperando por um tempo determinado
            Thread.sleep(2000);
        }catch (InterruptedException e) {
            e.printStackTrace();
        }
